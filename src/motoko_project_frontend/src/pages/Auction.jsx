...
const Auction = () => {
	...
	const handlegetAuction = async () => {
		setIsLoading(true);
		const data = await motoko_project_backend.getAuctionInfo(Number(id));
		const bids = await motoko_project_backend.getAuctionPastBids(Number(id));
		setIsLoading(false);
	...
	const handleNewBid = async (id, prize) => {
		...
		if (amount) {
			setIsLoading(true);
			const bid = await motoko_project_backend.newBid(
				Number(id),
				Number(amount)
			);
			setIsLoading(false);
